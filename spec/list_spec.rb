...
describe(List) do
  ...
  describe('#tasks') do
    it('returns an array of tasks for that list') do
      test_list = List.new({:name => 'Epicodus stuff', :id => nil})
      test_list.save()
      test_task = Task.new({:description => "Learn SQL", :due => '2016-07-12', :list_id => test_list.id()})
      test_task.save()
      test_task2 = Task.new({:description => "Review Ruby", :due => '2016-07-13', :list_id => test_list.id()})
      test_task2.save()
